...
🧠 We've trained two different models — a <b>realistic lap-time based model</b> and a <b>model that's weighted on important specific features</b> — and <u>blended their predictions</u> to form an ensemble that gives higher confidence in the winner prediction.<br><br>
🏆 The goal? Accurately predict who finishes first based on speed, consistency, experience, and more!<br><br>""", unsafe_allow_html=True)
...
if st.button("🏁 Predict Race Winner"):
   ...
   # for now prediction1.py the results that we get by training model in different way on less parameters will be using that here
    ...
    results_df, shap_values_positive, model_score, shap_fig, resume_metrics = run_prediction()
    ...
    with col1:
        st.metric(
            ...
            value=f"{resume_metrics['accuracy']:.1%}",
            help="5-fold cross-validation accuracy - most reliable metric"
        )

    with col2:
        st.metric(
            label="Precision Score", 
            value=f"{resume_metrics['precision']:.3f}",
            help="How many predicted top-3 finishes were actually correct"
        )

    with col3:
        st.metric(
            label="F1-Score", 
            value=f"{resume_metrics['f1_score']:.3f}",
            help="Balanced measure of precision and recall"
        )

    # Add a nice info box explaining the metrics
    st.info(f"""
    🎯 **Model Performance Summary**: 
    This classification model achieves **{resume_metrics['accuracy']:.1%} accuracy** in predicting which drivers will finish in the **Top 3** positions, 
    with a precision of **{resume_metrics['precision']:.3f}** and F1-score of **{resume_metrics['f1_score']:.3f}**.
    """)

    # Optional: Add a performance interpretation
    if resume_metrics['accuracy'] > 0.85:
        st.success("🔥 Excellent model performance!")
    elif resume_metrics['accuracy'] > 0.75:
